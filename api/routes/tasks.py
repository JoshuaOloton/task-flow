...
def get_tasks_paginated(
    ...
    return TaskService.get_tasks_paginated(db, current_user, skip=skip, limit=limit, status_query=status, priority=priority)


# GET /tasks/{task_id}
@task_router.get("/{task_id}", response_model=TaskResponse)
def get_task(task_id: str, db: Session = Depends(get_db), current_user: User = Depends(AuthService.get_current_user)):
